...
import NikshayDetailsForm, { NikshayDetailsData } from "../NikshayDetailsForm/NikshayDetailsForm";
import PatientDetailsForm, { PatientDetailsData } from "../PatientDetailsForm/PatientDetailsForm";
...
const EditPatientDetailsModal = ({ open, onClose, prop, patientId }: any) => {
	...
	const auth = useAuth();
	const userEmail =
		useSelector((state) => state.user?.profile?.email) || auth.user?.profile?.email;
	...
	const fetchFormData = async () => {
		...
		try {
			...
			setError(error.response?.data || "Failed to fetch form data. Please try again.");
		} finally {
			setLoading(false);
		}
	};
	...
	const handleUpdate = async (updatedData: any) => {
		...
		try {
			...
			setError(error.response?.data || "Failed to save data. Please try again.");
		} finally {
			setUpdating(false);
		}
	};
