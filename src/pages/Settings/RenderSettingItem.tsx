...
import { fetchMedications, fetchStates } from "./settingsService";
...
const RenderSettingItem = ({
  ...
  useEffect(() => {
    const loadOptions = async () => {
      ...
      } else if (setting.keyName === "state") {
        const stateListFromBE = await fetchStates();
        const stateCodes = stateListFromBE.map((state) => state.stateCode);

        const states = stateList.filter(
          (state) => !stateCodes.includes(state.value.stateCode)
        );
        setOptions(states);
      }
    };
          ...
          <Select
            ...
            onChange={(event) =>
              setValue(setting.keyName, {
                stateName:
                  event.target.selectedOptions[0].textContent.toUpperCase(),
