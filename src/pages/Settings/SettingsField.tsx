...
const SettingField = ({ setting, control, errors, setValue, reset }) => {
	...
	const insertTextAtCursor = (textToInsert: string, field) => {
		...
		const newValue =
			field.value.substring(0, start) + textToInsert + field.value.substring(end);
	...
	useEffect(() => {
		setEdited(() => {
			if (setting.type === "chips") return fieldValue.toString() !== setting.value.toString();
	...
	return (
		<Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
			...
			<Box sx={{ flex: 1, display: "flex", gap: 1 }}>
				{setting.type === "button" ? (
					<Button variant="contained" onClick={() => window.open(setting.value)}>
						{setting.placeholder}
					</Button>
				) : (
					<>
